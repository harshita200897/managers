...
#listwise deletion (deletes all rows with an NA)
...
#list data that does not have missing values
complete_data <- managers_data[complete.cases(managers_data),]
complete_data

#find sum of missing values in age attribute
sum(is.na(managers_data$Age))

sum(complete.cases(managers_data$Age)) #complete cases for age column


mean(is.na(managers_data$Age)) #calculates proportion mean of missing values in age attribute

install.packages("mice")
library(mice)
md.pattern(managers_data)

#sue the VIM packages
